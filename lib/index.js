...
class SpaceKitService {
  ...
  handleRelayConnection (webSocket, hostname) {
    ...
    if (this.dynamicDns) {
      // TODO: only perform dns resolution of this.config.apiHostname once
      Dns.resolve4(this.apiHostname, (err, addresses) => {
        if (err) {
          log.error(err, 'dns resolve error');
          // TODO: send error to the client (maybe disconnect)
        } else {
          let options = {
            hostname: hostname,
            recordType: 'A',
          ...
          this.dynamicDns.upsert(options, (err, data) => {
            if (err) {
              // TODO: send error to the client (maybe disconnect)
              return log.error(err, 'dynamic dns error');
            }
