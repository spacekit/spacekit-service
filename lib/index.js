...
class SpaceKitService {
  constructor (config) {
    ...
    if (config.apiKey || config.webKey) {
      ...
      let tlsServer = CreateTlsProxyServer(this.handleTlsConnection.bind(this, 443));
      tlsServer.listen(443);
      this.servers.push(tlsServer);

      for (let port = 8000; port <= 8999; port++) {
        let additionalServer = CreateTlsProxyServer(this.handleTlsConnection.bind(this, port));
        additionalServer.listen(port);
        this.servers.push(additionalServer);
      }

      // Listen for any incoming Net connections.
      let plainServer = CreateNetProxyServer(this.handlePlainTextConnection.bind(this));
      plainServer.listen(config.tcpPort);
      this.servers.push(plainServer);
    } else {
      let plainServer = CreateNetProxyServer(this.handleTlsConnection.bind(this, config.tcpPort));
      plainServer.listen(config.tcpPort);
      this.servers.push(plainServer);
    }
    ...
    if (config.apiKey) {
      ...
      this.apiServer = Http.createServer(ApiApp(config));
    }
    ...
    if (config.webKey) {
      ...
      this.webServer = Http.createServer(WebApp(config));
    }
