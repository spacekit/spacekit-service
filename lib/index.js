...
class SpaceKitService {
  ...
  /**
   * Handle non-tls (insecure) connections.
   *
   * If this connection's hostname is SpaceKit's root hostname, we redirect to
   * the website hostname. Otherwise, we redirected to the secure (https)
   * equivalent.
   */
  handleNetConnection (socket, hostname, path) {
    ...
    if (hostname === this.config.host) {
      response += `Location: https://${this.webHostname}${path}\r\n\r\n`;
      socket.end(response);
    } else {
      response += `Location: https://${hostname}${path}\r\n\r\n`;
      socket.end(response);
    }
  }
